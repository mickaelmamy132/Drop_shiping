...
function ChatModal({ productId, buyerId, sellerId,Produit_image, isOpen, onClose }) {
  ...
  const [selectedImage, setSelectedImage] = useState(null); // Stocke le fichier sélectionné
  ...
  const handleImageSelect = (event) => {
    ...
    if (file) {
      ...
      setImagePreview(URL.createObjectURL(file)); // Génère un aperçu local
    }
  };
  ...
  const handleSendImage = async () => {
    if (selectedImage) {
      ...
      try {
        ...
        await handleSendMessage('', 'image', imageUrl); // Envoie le message avec l'URL de l'image
